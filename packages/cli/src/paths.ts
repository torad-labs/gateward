 ...
 * publish itself: the Python engine (`packages/core/src/`) and the rule
 * packs (`packs/`), plus optional per-harness shims (`packages/shims/`).
 *
 * Paths are resolved relative to *this module's own compiled location*
 * (`__dirname`, three levels up from `dist/`), never `process.cwd()` —
 * `process.cwd()` is reserved for "the target project" (where `.tenets/`
 * gets written). This lets the built CLI be invoked with any working
 * directory (as the integration test does, from a temp dir) while still
 * finding its own vendored sources.
 *
 ...
 * pre-publish). Publishing this package standalone to npm will need a
 * packaging step that bundles core/src, packs/, and packages/shims into the
 * published tarball and updates these paths accordingly — out of scope here
 * (it would touch packages/core and add a build/publish pipeline, neither
 * requested by the pinned spec).
 */
...
export function readCliVersion(): string {
  const pkg = JSON.parse(fs.readFileSync(CLI_PACKAGE_JSON, "utf8")) as { version: string };
  return pkg.version;
}
